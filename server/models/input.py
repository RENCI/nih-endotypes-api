...
class Input(Model):
    ...
    """
    def __init__(self, date_of_birth: str=None, race: str=None, sex: str=None, model_type: str=None, visits: List[Visit]=None):
        """
    ...
    def race(self) -> str:
        ...
        race code ranged from 1 to 6 with the following mapping: 1: WHITE OR CAUCASIAN 2: BLACK OR AFRICAN AMERICAN 3: AMERICAN INDIAN OR ALASKA NATIVE 4: ASIAN 5: NATIVE HAWAIIAN OR OTHER PACIFIC ISLANDER 6: OTHER/HISPANIC 
    ...
    def sex(self, sex: str):
        ...
        :type sex: str
        """
        if sex is not None and not re.search('^(F|M|f|m)$', sex):
            raise ValueError("Invalid value for `sex`, must be a follow pattern or equal to `/^(F|M|f|m)$/`")
    ...
    def model_type(self, model_type: str):
        ...
        if model_type is not None and not re.search('^[M|m][0-9]$', model_type):
            raise ValueError("Invalid value for `model_type`, must be a follow pattern or equal to `/^[M|m][0-9]$/`")
    ...
    def visits(self) -> List[Visit]:
        """
        Gets the visits of this Input.

        :return: The visits of this Input.
        :rtype: List[Visit]
        """
        return self._visits
